...
void tx_application_define_user (void)
{
    ...
    UINT status_application_thread;

    status_application_thread = tx_thread_create(&application_thread, "Application Thread", application_thread_entry, 0, application_thread_memory_stack, 4096, 5, 5, 0, TX_AUTO_START);

    if (TX_SUCCESS != status_application_thread)
    {
        while (1)
        {
