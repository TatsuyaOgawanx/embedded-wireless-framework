...
#define OB1203_BIO_MAX_COUNTS (10) //10
...
//TickType_t Semphr_wait_ticks3 = pdMS_TO_TICKS ( 500 );//MS
...
static bool ob1203_bio_irq_callback_wait(void);
#endif
...
void ob1203_comms_i2c_callback(rm_ob1203_callback_args_t *p_args)
{
    ...
    //xSemaphoreGiveFromISR (g_obsensors_semaphore, &xHigherPriorityTaskWoken1);
}
...
bool ob1203_bio_measurement_period_wait(ob1203_bio_t *const p_bio)
{
    ...
    /* Wait IRQ callback */
    bstatus = ob1203_bio_irq_callback_wait();
    ...
    *p_update = false;
    ...
    if (2 <= p_bio->number_of_samples)
    {
        ...
        result = ob1203_bio_each_led_gain_current_control(p_bio,
                                                          ...
                                                          &ir_update);
//        printf("\r\n Sample pair 2+ %d",p_bio->number_of_samples);
//        printf("\r\n IR LED %d",result);
//        printf("\r\n IR update %d",ir_update);
        if (false == result)
        {
            return false;
        }
...
static bool ob1203_bio_irq_callback_wait()
{
//    while (OB1203_BIO_CALLBACK_STATUS_WAIT == gs_irq_callback_status)
//    {
//        // nothing
//    }
       if (OB1203_BIO_CALLBACK_STATUS_WAIT == gs_irq_callback_status)
       {
           return false;
       }
       else
