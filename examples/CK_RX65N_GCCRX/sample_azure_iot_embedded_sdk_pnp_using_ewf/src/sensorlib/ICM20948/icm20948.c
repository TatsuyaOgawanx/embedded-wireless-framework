...
void readicm20948dta()
{
	...
	configPRINTF( ( "Acc Corrected : x: %f, y: %f,  z: %f \r\n",corrAccRaw.x,corrAccRaw.y,corrAccRaw.z) );
	configPRINTF( ( "G values : x: %f, y: %f,  z: %f \r\n",gVal.x,gVal.y,gVal.z) );
	configPRINTF( ( "Resultant G : %f\r\n",resultantG ));
	configPRINTF( ( "Gyro Data : x: %f, y: %f,  z: %f \r\n",gyr.x,gyr.y,gyr.z) );
	configPRINTF( ( "Mag Data : x: %f, y: %f,  z: %f \r\n",magValue.x,magValue.y,magValue.z) );
#endif
	vTaskDelay(100);
