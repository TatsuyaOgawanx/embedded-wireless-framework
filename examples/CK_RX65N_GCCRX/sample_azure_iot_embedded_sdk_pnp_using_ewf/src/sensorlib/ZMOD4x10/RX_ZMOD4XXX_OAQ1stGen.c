...
//TickType_t Semphr_wait_ticks1 = pdMS_TO_TICKS ( 500 );//MS
   ...
   // zmod4410sens->zmod410_oaq=0;
    switch(sequence_no)
    {
        case ZMOD4510_DEMO_SEQUENCE_1 :
        {
            ...
            {
            	printf ("Task zmod4510 task delete in seq 1:%d\r\n",err);
            }
        }
        break;
...
//        if( pdPASS==xSemaphoreTake( g_allsensors_semaphore , Semphr_wait_ticks1 ))
//        {
...
//        	{
            switch (g_i2c_callback_status1)
            {
                ...
                case ZMOD4510_DEMO_CALLBACK_STATUS_REPEAT :
                    ...
                    break;
                default :
                	printf ("Task zmod4510 task delete in seq 6:%d\r\n",err);
                    break;
            }
        	//}
	...
	//configPRINTF( ( "ZMOD4510-OAQ  : %f \r\n",gs_demo_air_quality) );
#endif
            }
            else if (FSP_ERR_SENSOR_IN_STABILIZATION == err)
            {
            	printf ("ZMOD4510 in stabilization:%d\r\n",err);
                sequence_no = ZMOD4510_DEMO_SEQUENCE_1;
            }
            else
            {
            	printf ("Task zmod4510 task delete in seq 7:%d\r\n",err);
            }
        ...
        default :
        {
        	printf ("Task zmod4410 task delete in default case:%d\r\n",err);
        }
        break;
    }
}
